...
class AppState extends ChangeNotifier {
  ...
  Future<void> load() async {
    ...
    categories
      ..clear()
      ..addAll((prefs.getStringList('categories') ?? []).where((e) => e.trim().isNotEmpty));
    invoices
      ..clear()
    ...
    if (settingsRaw != null && settingsRaw.isNotEmpty) {
      repSettings = RepSettings.fromJson(Map<String, dynamic>.from(jsonDecode(settingsRaw) as Map));
    }

    if (products.isEmpty && customers.isEmpty && suppliers.isEmpty && invoices.isEmpty) {
      _seed();
      await save();
    } else {
  ...
  Future<void> importData(Map<String, dynamic> data) async {
    customers
      ..clear()
      ..addAll(((data['customers'] as List?) ?? []).map((e) => Customer.fromJson(Map<String, dynamic>.from(e as Map))));
    suppliers
      ..clear()
      ..addAll(((data['suppliers'] as List?) ?? []).map((e) => Supplier.fromJson(Map<String, dynamic>.from(e as Map))));
    products
      ..clear()
      ..addAll(((data['products'] as List?) ?? []).map((e) => Product.fromJson(Map<String, dynamic>.from(e as Map))));
    categories
      ..clear()
      ..addAll((((data['categories'] as List?) ?? [])).map((e) => e.toString()));
    invoices
      ..clear()
      ..addAll(((data['invoices'] as List?) ?? []).map((e) => Invoice.fromJson(Map<String, dynamic>.from(e as Map))));
    payments
      ..clear()
      ..addAll(((data['payments'] as List?) ?? []).map((e) => InvoicePayment.fromJson(Map<String, dynamic>.from(e as Map))));
    repSettings = RepSettings.fromJson(Map<String, dynamic>.from((data['repSettings'] as Map?) ?? <String, dynamic>{}));
    _migratePaymentsIfNeeded();
    _normalizeInvoiceStatuses();
    await save();
  }
  ...
  Future<void> save() async {
    ...
    await prefs.setString('suppliers', jsonEncode(suppliers.map((e) => e.toJson()).toList()));
    await prefs.setString('products', jsonEncode(products.map((e) => e.toJson()).toList()));
    await prefs.setStringList('categories', categories);
    await prefs.setString('invoices', jsonEncode(invoices.map((e) => e.toJson()).toList()));
    await prefs.setString('payments', jsonEncode(payments.map((e) => e.toJson()).toList()));
    await prefs.setString('repSettings', jsonEncode(repSettings.toJson()));
    notifyListeners();
  }
  ...
  String newId() => '${DateTime.now().microsecondsSinceEpoch}${Random().nextInt(999)}';
  ...
  Future<void> addProduct(Product item) async {
    ...
    if (item.category.trim().isNotEmpty && !categories.contains(item.category.trim())) {
  ...
  Future<void> deleteCustomer(String id) async {
    ...
    final customerInvoiceIds = invoices.where((e) => e.customerId == id).map((e) => e.id).toList();
    payments.removeWhere((e) => e.customerId == id || customerInvoiceIds.contains(e.invoiceId));
  ...
  Future<void> addInvoice({
    ...
    final subtotal = normalizedItems.fold<double>(0, (sum, item) => sum + item.total);
    final discountValue = discountIsPercent ? (subtotal * (discount / 100)) : discount;
    final safeDiscount = discountValue.clamp(0, subtotal).toDouble();
    final safeTaxValue = taxValue < 0 ? 0 : taxValue;
    final total = (subtotal - safeDiscount + safeTaxValue).clamp(0, double.infinity).toDouble();
    final safePaid = paidAmount.clamp(0, total).toDouble();
    ...
    requested.forEach((productId, qty) {
      final product = products.cast<Product?>().firstWhere((e) => e?.id == productId, orElse: () => null);
      if (product == null || product.stock < qty) {
        insufficient.add(product?.name ?? 'منتج غير معروف');
      }
    });
    if (insufficient.isNotEmpty) {
      throw StateError('الكمية المتاحة غير كافية للمنتجات: ${insufficient.join('، ')}');
    }
    ...
    final preview = Invoice(
      ...
      discount: discountIsPercent ? (safeDiscount == 0 || subtotal == 0 ? discount : (safeDiscount / subtotal) * 100) : safeDiscount,
  ...
  double totalFrom(List<InvoiceItem> items, double taxValue, double discount, bool discountIsPercent) {
    final subtotal = items.fold<double>(0, (sum, item) => sum + item.total);
    final discountValue = discountIsPercent ? (subtotal * (discount / 100)) : discount;
    final safeDiscount = discountValue.clamp(0, subtotal).toDouble();
    final safeTax = taxValue < 0 ? 0 : taxValue;
    return (subtotal - safeDiscount + safeTax).clamp(0, double.infinity).toDouble();
  }
  ...
  Future<void> addPayment({
    ...
    final safeAmount = amount.clamp(0, invoice.remaining).toDouble();
    if (safeAmount <= 0) {
      throw StateError('المبلغ المدخل غير صالح');
    }
    final newPaid = (invoice.paidAmount + safeAmount).clamp(0, invoice.total).toDouble();
    ...
    payments.insert(
      ...
      InvoicePayment(
        ...
        amount: safeAmount,
        notes: notes.trim().isEmpty ? 'دفعة للفاتورة رقم ${invoice.number}' : notes.trim(),
  ...
  void _applyPaymentOnInvoice(InvoicePayment payment) {
    ...
    final invoice = invoices[invoiceIndex];
    final newPaid = (invoice.paidAmount + payment.amount).clamp(0, invoice.total).toDouble();
    invoices[invoiceIndex] = invoice.copyWith(
      paidAmount: newPaid,
      status: _resolveStatus(invoice.total, newPaid),
    );
  ...
  void _rollbackPaymentOnInvoice(InvoicePayment payment) {
    ...
    if (invoiceIndex == -1) return;
    final invoice = invoices[invoiceIndex];
    final newPaid = max(0, invoice.paidAmount - payment.amount).toDouble();
    invoices[invoiceIndex] = invoice.copyWith(
      paidAmount: newPaid,
      status: _resolveStatus(invoice.total, newPaid),
    );
  ...
  double get totalSales => invoices.fold(0, (sum, inv) => sum + inv.total);
  double get totalPaid => invoices.fold(0, (sum, inv) => sum + inv.paidAmount);
  double get totalOutstanding => invoices.fold(0, (sum, inv) => sum + max(0, inv.remaining));
  int get invoiceCount => invoices.length;
  int get customerCount => customers.length;
  int get productCount => products.length;

  int invoiceCountForCustomer(String customerId) => invoices.where((e) => e.customerId == customerId).length;

  double outstandingForCustomer(String customerId) =>
      invoices.where((e) => e.customerId == customerId).fold<double>(0, (sum, inv) => sum + max(0, inv.remaining));

  double totalSalesForCustomer(String customerId) =>
      invoices.where((e) => e.customerId == customerId).fold<double>(0, (sum, inv) => sum + inv.total);

  double totalPaidForCustomer(String customerId) =>
      invoices.where((e) => e.customerId == customerId).fold<double>(0, (sum, inv) => sum + inv.paidAmount);
  ...
  void _normalizeInvoiceStatuses() {
    for (var i = 0; i < invoices.length; i++) {
      ...
      final paid = invoice.paidAmount.clamp(0, invoice.total).toDouble();
  ...
  void _seed() {
    ...
    final c1 = Customer(id: newId(), name: 'عميل نقدي', phone: '01094932665', email: '', address: 'بني عبيد دقهلية');
    final s1 = Supplier(id: newId(), name: 'مورد افتراضي', phone: '01111111111', address: 'الجيزة');
    categories.addAll(['عام', 'إكسسوارات']);
    final p1 = Product(id: newId(), name: 'كليفر بيوني', unit: 'قطعة', price: 125, stock: 1000, category: 'عام', barcode: '123456789');
    customers.add(c1);
    suppliers.add(s1);
    products.add(p1);
    final seededInvoice = Invoice(
      ...
      createdAt: DateTime.now(),
      items: [
        InvoiceItem(productId: p1.id, name: p1.name, price: p1.price, quantity: 50),
      ],
      taxPercent: 0,
      taxValue: 0,
      discount: 0,
